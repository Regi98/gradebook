...
$(function () {
    // Initialize Firebase
  var config = {
    apiKey: "AIzaSyCF9vvNXQ2Ghj1gMxni_eR5vdDmSF7tpuI",
    authDomain: "jcfcdatabase.firebaseapp.com",
    databaseURL: "https://jcfcdatabase.firebaseio.com",
    projectId: "jcfcdatabase",
    storageBucket: "jcfcdatabase.appspot.com",
    messagingSenderId: "299393731031"
  };
...
//DATA RETRIEVAL w/ Jquery
  ...
  rootRef.on("child_added", snap => {
    ...
    $("#datatable-buttons").DataTable().row.add([parentKey,sn, name, pass, lvl, section, '<td><i class="fa fa-pencil-square" aria-hidden="true"></i>&nbsp;&nbsp;<i class="fa fa-minus-square" aria-hidden="true"></i></td>']).draw();
    });
  ...
  rootRef.on("child_changed", snap => {
    ...
    rootRef.orderByChild("SN").equalTo(sn).on("child_changed", function(snapshot) {
      var parentKey = snapshot.key;
    $("#datatable-buttons").DataTable().row().data([parentKey,sn, name, pass, lvl, section, '<td><i class="fa fa-pencil-square" aria-hidden="true"></i>&nbsp;&nbsp;<i class="fa fa-minus-square" aria-hidden="true"></i></td>']).draw();
    });
...
// START ADD ROW STUDENT
    ...
    $("#datatable-buttons").on('mousedown.add', "i.fa.fa-plus", function(e) {
        ...
        //Array of entities
          var entity = ["SN","fName", "password", "grLevel", "section" ];
        //Update in database
          rootRef.push({
              [entity[0]]: txtSN,
              [entity[1]]: txtfName,
...
// START DATA EDIT
      ...
      $("#datatable-buttons").on('mousedown.edit', "i.fa.fa-pencil-square", function(e) {
        ...
        var selectedSection = $row.find('td:nth-child(6)').html();
         $row.find('td:nth-child(6)').html("").append('<select id="selectSection" class="form-control"><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select>');
      ...
      $("#datatable-buttons").on('mousedown.save', "i.fa.fa-envelope-o", function(e) {
        ...
        //Update in database
          ...
          var selectedSection = $('#selectSection').find(":selected").text();
          var pushedRef = firebase.database().ref('students/7' + sectionChild+'/' + parentKey).update({ [entity[4]]: selectedSection }); 
          $row.find('td:nth-child(6)').html(selectedSection);

          if(selectedSection != sectionChild) {
            //get value of each td
              var tdSN = $row.find('td:nth-child(2)').html();
              var tdfName = $row.find('td:nth-child(3)').html();
              var tdPassword = $row.find('td:nth-child(4)').html();
              var tdGrlevel = $row.find('td:nth-child(5)').html();
              var tdSection = $row.find('td:nth-child(6)').html();
            //Array of entities
              var entity = ["SN","fName", "password", "grLevel", "section" ];
            //Update in database
